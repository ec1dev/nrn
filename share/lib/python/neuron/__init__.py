...
import sys as _interface_sys

_interface_choice = _interface_os.environ.get("NEURON_PYTHON_INTERFACE", "legacy")
if _interface_choice not in {"legacy", "myneuron"}:
    raise ImportError(f"unknown NEURON_PYTHON_INTERFACE={_interface_choice!r}")
if globals().get("_selected_interface", _interface_choice) != _interface_choice:
    raise ImportError("changing NEURON interfaces requires a fresh Python process")
if _interface_choice == "legacy" and "myneuron" in _interface_sys.modules:
    raise ImportError("legacy NEURON cannot initialize after myneuron; start a fresh process")
_selected_interface = _interface_choice

if _interface_choice == "myneuron":
    from ._myneuron_interface import populate as _populate_interface
    _populate_interface(globals())
    del _populate_interface
else:
    import sys
    import os
    import warnings
